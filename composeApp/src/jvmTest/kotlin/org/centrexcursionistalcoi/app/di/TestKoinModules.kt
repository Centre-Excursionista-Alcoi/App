...
/**
 * Verifies the whole annotation-driven Koin graph (the `@ComponentScan`-based modules in `AnnotatedModules.kt`)
 * actually resolves at runtime -- compiling clean isn't enough here, since a constructor-order mismatch between
 * an `@Singleton` class and what `@ComponentScan` derives from it would only surface as a Koin runtime error, not
 * a compile error. This is also the only safety net for missing/incorrect bindings at all: `compileSafety` is off
 * (see the `koinCompiler { }` block in `composeApp/build.gradle.kts`) because the plugin's own compile-time graph
 * verification currently false-positives on this project.
 */
class TestKoinModules {
    ...
    @Test
    fun `annotated repositories, remote repositories and services resolve`() {
        val koin = startKoin {
            modules(
